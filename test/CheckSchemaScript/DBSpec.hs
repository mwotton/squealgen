...
spec = describe "check_schema/buildTestSchema scripts" $ do
  -- Integration-first checks execute the real repository scripts; mock-heavy tests below
  -- remain for fast, contract-level assertions where full toolchain setup would be expensive.
  it "make ci succeeds through real drift and coverage scripts with a minimal stubbed toolchain" $ do
    repoRoot <- getCurrentDirectory
    withSystemTempDirectory "make-ci-real-scripts" $ \tmpDir -> do
      let makefile = tmpDir </> "Makefile"
          ...
          generatedPath = tmpDir </> "squealgen"
      ...
      writeFile fakeHpc $ unlines
        ...
        , "fi"
        , "if [[ \"$1\" == \"show\" ]]; then"
        , "  printf '%s\\n' \"0 1 pkg:Foo 3:1-3:5 ExpBox False\""
        , "  exit 0"
        , "fi"
      ...
      env <- (("FAKE_TEST_BIN", fakeTestBin) :) . overridePath fakeBin <$> getEnvironment
      let cmd = (proc "bash" ["-lc", "cd \"" <> tmpDir <> "\" && make ci"]) { env = Just env }
      (exitCode, _, err) <- readCreateProcessWithExitCode cmd ""
      exitCode `shouldBe` ExitSuccess
      err `shouldBe` ""
      doesFileExist generatedPath `shouldReturn` True
      generated <- readFile generatedPath
      generated `shouldSatisfy` ("NB: this file is generated by mksquealgen.sh" `isInfixOf`)
      summary <- readFile summaryPath
      summary `shouldSatisfy` ("coverage_gate_result=pass" `isInfixOf`)
      summary `shouldSatisfy` ("included_modules=Foo" `isInfixOf`)
  ...
  it "make ci executes drift check and coverage in order without a separate cabal test pass" $ do
    -- Contract-level test: validates target ordering and prevents a direct cabal test pass
    -- from sneaking into make ci while keeping runtime very small.
    repoRoot <- getCurrentDirectory
